...
[HashTemplate](https://github.com/nathan-russell/hashmap/blob/master/inst/include/hashmap/HashTemplate.hpp), leveraging the functionality of the C++98-compliant 
...
`environment` object against `hashtable` for 

1. Construction of the hash table 
2. Vectorized key lookup 

An overview of results in presented here, but the 
full code to reproduce the test is in `assets/benchmark.R`. All 
of the tests use a one million element character vector for 
...
plotted below on the linear and logarithmic scale:
...
The benchmark test was conducted on a laptop running Ubuntu 
